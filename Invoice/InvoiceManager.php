...
use Sulu\Bundle\Sales\ShippingBundle\Api\Invoice;
...
use Sulu\Bundle\Sales\ShippingBundle\Shipping\Exception\MissingInvoiceAttributeException;
...
{
    protected static $invoiceEntityName = 'SuluSalesInvoiceBundle:Invoice';
    protected static $invoiceStatusEntityName = 'SuluSalesInvoiceBundle:InvoiceStatus';
    protected static $invoiceStatusTranslationEntityName = 'SuluSalesInvoiceBundle:InvoiceStatusTranslation';
...
//        }
    ...
    {
        $invoice = $this->em->getRepository(self::$invoiceEntityName)->findByIdAndLocale($id, $locale);

        if (!$invoice) {
            return null;
        }
        $apiInvoice = new Invoice($invoice, $locale);

        // set currently shipped items (sum of all shippings for that item)
        $sumBilledItems = $this->getSumOfBilledItemsByOrderId($invoice->getOrder()->getId());
        foreach ($apiInvoice->getItems() as $apiItem) {
            $itemId = $apiItem->getEntity()->getItem()->getId();
            if (array_key_exists($itemId, $sumBilledItems)) {
                $apiItem->setShippedItems($sumBilledItems[$itemId]);
            }
        }
        return $apiInvoice;
    }
    ...
    /**
     * returns the sum of all billed items by order id
     * @param $orderId
     * @return array
     */
    public function getSumOfBilledItemsByOrderId($orderId)
    {
        $result = array();
        $sums = $this->em->getRepository(self::$invoiceEntityName)->getSumOfBilledItemsByOrderId($orderId);
        foreach ($sums as $sum) {
            $result[$sum['items_id']] = intval($sum['billed']);
        }
        return $result;
    }
    ...
    {
        if (empty($filter)) {
            $invoice = $this->em->getRepository(self::$invoiceEntityName)->findAllByLocale($locale);
        } else {
            $invoice = $this->em->getRepository(self::$invoiceEntityName)->findByLocaleAndFilter($locale, $filter);
        }

        array_walk(
            $invoice,
            function (&$invoice) use ($locale){
                $invoice = new Invoice($invoice, $locale);
            }
        );
    ...
    private function initializeFieldDescriptors($locale)
    {
        ...
        $this->fieldDescriptors['id'] = new DoctrineFieldDescriptor('id', 'id', self::$invoiceEntityName, 'public.id', array(), true);
        $this->fieldDescriptors['number'] = new DoctrineFieldDescriptor('number', 'number', self::$invoiceEntityName, 'salesinvoice.invoice.number', array(), false, true);

        $contactJoin = array(
            self::$orderAddressEntityName => new DoctrineJoinDescriptor(
                    self::$orderAddressEntityName,
                    self::$invoiceEntityName . '.invoiceAddress'
                )
        );
        ...
        $this->fieldDescriptors['status'] = new DoctrineFieldDescriptor(
            'name',
            'status',
            self::$invoiceStatusTranslationEntityName,
            'salescore.status',
            array(
                self::$invoiceStatusEntityName => new DoctrineJoinDescriptor(
                        self::$invoiceStatusEntityName,
                        self::$invoiceEntityName . '.status'
                    ),
                self::$invoiceStatusTranslationEntityName => new DoctrineJoinDescriptor(
                        self::$invoiceStatusTranslationEntityName,
                        self::$invoiceStatusEntityName . '.translations',
                        self::$invoiceStatusTranslationEntityName . ".locale = '" . $locale . "'"
                    )
            )
        );
     ...
     * @param $isNew
     * @return bool
     * @throws Exception\MissingInvoiceAttributeException
     */
    private function checkDataSet(array $data, $key, $isNew)
    {
        ...
        if (($isNew && !($keyExists && $data[$key] !== null)) || (!$keyExists || $data[$key] === null)) {
            throw new MissingInvoiceAttributeException($key);
        }
    ...
    private function setTermsOfDelivery($data, Shipping $shipping)
    {
        ...
        if ($termsOfDeliveryData) {
            if (!array_key_exists('id', $termsOfDeliveryData)) {
                throw new MissingInvoiceAttributeException('termsOfDelivery.id');
            }
    ...
    private function setShippingOrder($data, Shipping $shipping)
    {
        ...
        if ($orderData) {
            if (!array_key_exists('id', $orderData)) {
                throw new MissingInvoiceAttributeException('order.id');
            }
        ...
        try {
            if ($this->checkDataSet($data,'items', false)) {
                ...
                if (!is_array($data['items'])) {
                    throw new MissingInvoiceAttributeException('items array');
                }
    ...
    private function createShippingItemEntity($data)
    {
        ...
        if (array_key_exists('item', $data) && array_key_exists('id', $data['item'])) {
            ...
            throw new MissingInvoiceAttributeException('ShippingItems.item.id');
        }
